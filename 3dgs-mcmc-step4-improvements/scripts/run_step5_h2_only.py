...
EXP = {
    ...
    "model": r"D:\AI WORK\3dgs-mcmc-step4-improvements\output\step5\train_h2_systematic_80cap_full",
...
def main():
    ...
    train_result = run_monitored(
        ...
        LOGS / f"step5_{name}_full_train.out.log",
        LOGS / f"step5_{name}_full_train.err.log",
        RESULTS / f"gpu_mem_{name}_full.csv",
    )
    append_row(
        TRAINING_CSV,
        [
    ...
    render_result = run_plain(
        ...
        LOGS / f"step5_{name}_full_render.out.log",
        LOGS / f"step5_{name}_full_render.err.log",
    )
    ...
    metrics_result = run_plain(
        ...
        LOGS / f"step5_{name}_full_metrics.out.log",
        LOGS / f"step5_{name}_full_metrics.err.log",
    )
